...
def train(df: pd.DataFrame, model_params, num_labels=3, use_gpu=False):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(model_params.huggingface, num_labels=num_labels)

    metric_string = "f1"
    train_rows = tokenized_dataset.num_rows["train"]
    num_train_epochs = 5
    eval_save_steps = train_rows // 2
    eval_save_steps_adj = eval_save_steps // model_params.gradient_accumulation_steps // model_params.batch_size
    warmup_steps = train_rows * num_train_epochs // model_params.gradient_accumulation_steps // model_params.batch_size // 100
