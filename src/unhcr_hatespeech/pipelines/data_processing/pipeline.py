"""
This is a boilerplate pipeline 'data_processing'
generated using Kedro 0.18.1
"""

from kedro.pipeline import Pipeline, node, pipeline

from .nodes import (
    clean_text,
    combine_training_datasets,
    preprocess_cad,
    # preprocess_civil,
    preprocess_davidson,
    preprocess_dynhs,
    preprocess_ghc,
    preprocess_hasoc,
    preprocess_hatemoji,
    preprocess_hateval,
    preprocess_hatexplain,
    preprocess_slur,
    preprocess_ousid,
    preprocess_wiki,
    preprocess_hatecheck,
    preprocess_unhcr,
    split_stratified_into_train_val_test,
    oversample,
)


def create_pipeline(**kwargs) -> Pipeline:
    return pipeline(
        [
            node(
                func=preprocess_cad,
                inputs="raw_train_cad",
                outputs="preprocessed_cad",
                name="preprocess_cad_node",
                namespace="raw",
            ),
            # node(
            #    func=preprocess_civil,
            #   inputs="raw_train_civil",
            #  outputs="preprocessed_civil",
            # name="preprocess_civil_node",
            # namespace="raw",
            # ),
            node(
                func=preprocess_davidson,
                inputs="raw_train_davidson",
                outputs="preprocessed_davidson",
                name="preprocess_davidson_node",
                namespace="raw",
            ),
            node(
                func=preprocess_dynhs,
                inputs="raw_train_dynhs",
                outputs="preprocessed_dynhs",
                name="preprocess_dynhs_node",
                namespace="raw",
            ),
            node(
                func=preprocess_ghc,
                inputs="raw_train_ghc",
                outputs="preprocessed_ghc",
                name="preprocess_ghc_node",
                namespace="raw",
            ),
            node(
                func=preprocess_hasoc,
                inputs="raw_train_hasoc",
                outputs="preprocessed_hasoc",
                name="preprocess_hasoc_node",
                namespace="raw",
            ),
            node(
                func=preprocess_hatemoji,
                inputs="raw_train_hatemoji",
                outputs="preprocessed_hatemoji",
                name="preprocess_hatemoji_node",
                namespace="raw",
            ),
            node(
                func=preprocess_hateval,
                inputs="raw_train_hateval",
                outputs="preprocessed_hateval",
                name="preprocess_hateval_node",
                namespace="raw",
            ),
            node(
                func=preprocess_hatexplain,
                inputs="raw_train_hatexplain",
                outputs="preprocessed_hatexplain",
                name="preprocess_hatexplain_node",
                namespace="raw",
            ),
            node(
                func=preprocess_ousid,
                inputs="raw_train_ousid",
                outputs="preprocessed_ousid",
                name="preprocess_ousid_node",
                namespace="raw",
            ),
            node(
                func=preprocess_slur,
                inputs="raw_train_slur",
                outputs="preprocessed_slur",
                name="preprocess_slur_node",
                namespace="raw",
            ),
            node(
                func=preprocess_wiki,
                inputs="raw_train_wiki",
                outputs="preprocessed_wiki",
                name="preprocess_wiki_node",
                namespace="raw",
            ),
            node(
                func=preprocess_hatecheck,
                inputs="raw_hatecheck",
                outputs="preprocessed_hatecheck",
                name="preprocess_hatecheck_node",
                namespace="raw",
            ),
            node(
                func=preprocess_unhcr,
                inputs="raw_unhcr",
                outputs="preprocessed_unhcr",
                name="preprocess_unhcr_node",
                namespace="raw",
            ),
            node(
                func=combine_training_datasets,
                inputs=[
                    "preprocessed_cad",
                    # "preprocessed_civil",
                    "preprocessed_davidson",
                    "preprocessed_dynhs",
                    "preprocessed_ghc",
                    "preprocessed_hasoc",
                    "preprocessed_hatexplain",
                    "preprocessed_hatemoji",
                    "preprocessed_hateval",
                    "preprocessed_ousid",
                    "preprocessed_slur",
                    "preprocessed_wiki",
                ],
                outputs="primary_combined",
                namespace="primary",
            ),
            node(
                func=clean_text,
                inputs="primary_combined",
                outputs="model_input_combined",
                namespace="primary",
                name="clean_model_input_text_node",
            ),
            node(
                func=clean_text,
                inputs="preprocessed_hatecheck",
                outputs="test_hatecheck",
                namespace="primary",
                name="clean_test_hatecheck_text_node",
            ),
            node(
                func=clean_text,
                inputs="preprocessed_unhcr",
                outputs="test_unhcr",
                namespace="primary",
                name="clean_test_unhcr_text_node",
            ),
            node(
                func=split_stratified_into_train_val_test,
                inputs="model_input_combined",
                outputs=["train_set", "validation_set", "test_set"],
                name="split_combined_input_dataset_into_train_val_and_test_set",
                namespace="model_input",
            ),
            node(
                func=oversample,
                inputs="train_set",
                outputs="train_set_oversampled",
                name="oversampling_the_train_set",
                namespace="model_input",
            ),
        ]
    )
